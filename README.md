...
`"pinned": true` を追加すると、並び替え条件（新しい順・古い順・再生数順など）に関係なく、その項目が一覧の一番先頭に固定表示されます。

`category` は文字列だけでなく配列も指定できます（例: `"category": ["受付業務", "分院・受付業務"]`）。複数指定すると、その項目はどちらのカテゴリで絞り込んでもヒットし、カード上にもタグが複数表示されます。

## PDFの追加方法

PDFファイルもGoogle Drive埋め込みでそのまま閲覧できます。動画と同様にファイルをアップロードして共有設定をしたら、`"type": "pdf"` を指定して `driveFileId` にPDFのファイルIDを入れてください。
...
```json
{
  ...
  "recordedDate": "",
  "type": "pdf",
  "driveFileId": "PDFのファイルID"
}
```

一覧のカードには「📄 PDF」のバッジが付き、詳細を開くとGoogle Driveのプレビューでページをめくりながら閲覧できます。
